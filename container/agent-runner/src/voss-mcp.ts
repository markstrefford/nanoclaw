/**
 * VOSS CRM MCP Server — v2 (Bun) port.
 *
 * Ported from NanoClaw v1 (container/agent-runner/src/voss-mcp.ts). Calls the
 * VOSS CRM HTTP API. Registered per-agent-group via `mcpServers` (only North
 * today) and launched as `bun /app/src/voss-mcp.ts`.
 *
 * Credentials: the real `X-API-Key` is NEVER present in the container. We send
 * the OneCLI placeholder `onecli-managed`; the OneCLI gateway matches the VOSS
 * host pattern and rewrites the header with the real key in flight. This is the
 * same stub pattern the Gmail tool uses. The only env var the container needs is
 * the backend URL (not a secret).
 */
...
const server = new McpServer({ name: 'voss-crm', version: '2.0.0' });
