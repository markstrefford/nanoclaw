...
export function getOutboundDb(): Database {
  if (!_outbound) {
    ...
    `);
    // token_usage: per-turn token + cost accounting from the provider SDK's
    // result message. Container-written, host-read (the host aggregates these
    // across sessions for the daily cost report). No seq parity needed — the
    // host never writes this table. Forward-compat for older outbound.db files.
    _outbound.exec(`
